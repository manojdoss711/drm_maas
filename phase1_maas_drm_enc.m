...
% There are catches in this script that prevent you from overwriting data 
% that have already been filled out. Specifically, the first encoding 
% session for a participant can always be run. Once it has been run, the 
% only way to run the second encoding session is by completing the first 
% retrieval session. In case of a crash in the middle of the script, you 
% can simply rerun the script, and it will pick up where it left off. 
% Subject number 666 can be used for piloting, which always begins from the
% beginning of the first encoding phase. You can also change the variable 
...
if ~isequal(subject,666)
    ...
    elseif isempty(retstim1{end,idx_stim1_onset})
        error('Unfinished data from first Phase 2. Finish first Phase 2.');
    elseif isempty(encstim2{end,idx_stim1_onset})
        encstim = encstim2;
    elseif ~isempty(encstim2{end,idx_stim1_onset})
        error('Data for second Phase 1 already exists. Choose different subject number.');
    end
...
try
    ...
    for itrial = trial_start:num_trial
        ...
        if encstim{itrial,idx_ses} == 1
            encstim1 = encstim;
        elseif encstim{itrial,idx_ses} == 2
            encstim2 = encstim;
        end
