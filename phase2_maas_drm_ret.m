...
% from phase 0. You will first be asked to provide a subject number. There 
% are catches in this script that prevent you from overwriting data that 
% have already been filled out. Specifically, the first retrieval session 
% for a participant can only be run if the first encoding session has been 
% filled out. The only way to run the second retrieval session is by 
% completing the first encoding session, the first retrieval session, and 
% the second encoding session. In case of a crash in the middle of the 
% script, you can simply rerun the script, and it will pick up where it 
% left off. Subject number 666 can be used for piloting, which always 
% begins from the beginning of the first retrieval phase. You can also 
% change the variable speed_mode at the top from 0 to 1 to allow all trials 
% to run rapidly all the way through, but unlike the encoding phase, these 
% trials require a response, so you will have to push responses. Debug_mode 
% can be changed from 0 to 1 to allow the screen to be small.
...
if ~isequal(subject,666)
    ...
    elseif isempty(encstim2{end,idx_stim1_onset})
        error('Unfinished data from second Phase 1. Finish second Phase 1.');
    elseif isempty(retstim2{end,idx_stim1_onset})
        retstim = retstim2;
    else
        error('Data for second Phase 2 already exists. Choose different subject number.');
    end
...
try
    ...
    for itrial = trial_start:num_trial
        ...
        if retstim{itrial,idx_ses} == 1
            retstim1 = retstim;
        elseif retstim{itrial,idx_ses} == 2
            retstim2 = retstim;
        end
